...
> {
  ...
  private handleInputChange = (e: ChangeEvent<HTMLInputElement>): void => {
    ...
    this.setState({
      [name]: value,
    } as unknown as Pick<RegisterUserState, keyof RegisterUserState>);
  };
  ...
  private handleSubmit = async (e: FormEvent): Promise<void> => {
    ...
    try {
      await this.authService.register({ name, email, phone, password, role });
      this.props.onRegisterSuccess();
    } catch (err: unknown) {
      let errorMsg = "Gagal mendaftarkan akun baru.";
  ...
  render() {
    const { name, email, phone, password, showPassword, isLoading, error } =
      this.state;
    ...
    return (
      <form onSubmit={this.handleSubmit} className="space-y-5">
        ...
        <div className="mt-6 flex flex-col gap-4 text-center text-sm font-medium">
          ...
          <div>
            <span className="text-gray-500">Ingin berjualan di kantin? </span>
            <Link
              to="/register-mitra"
